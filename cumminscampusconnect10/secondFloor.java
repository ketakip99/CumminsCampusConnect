...
public class secondFloor extends AppCompatActivity {
    ...
    private void handleTouch(int x, int y) {
        ...
        if (isInArea(x, y, 26, 1561, 141, 1675)) {
            showAlert("T11","Sandhya Arora");
        } else if (isInArea(x, y, 26, 773, 135, 893)) {
            showAlert("T12","Prachi Waghmare\nSA Mangale\nKS Joshi ");
        }else if (isInArea(x, y, 1496, 764, 1612, 886)) {
            showAlert("T13","Manasi Pathade\nSandhya Potdar\nVidya Sisale");
        }else if (isInArea(x, y, 1494, 1556, 1606, 1674)) {
            showAlert("T14","Ashutosh Muchrikar");
        }else if (isInArea(x, y, 992, 1784, 1397, 1919)) {
            showAlert("T15","RR Borhade\nNutan Deshmukh\nVarsha Pimpale\nNilfar Kittad\nG Phandikar\nJyoti Bangare\nPrakash Date");
        }
